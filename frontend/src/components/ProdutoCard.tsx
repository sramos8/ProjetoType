...
export function ProdutoCard({ produto, onEditar, onDeletar, onToggleDisponivel }: ProdutoCardProps) {
  ...
  const [lotesAberto, setLotesAberto] = useState(false);
  ...
  return (
    <div style={{ ...styles.card, opacity: produto.disponivel ? 1 : 0.65 }}>
      ...
      <div style={styles.header}>
        ...
        <span style={{
          ...styles.badge,
          background: produto.disponivel ? '#D4EDDA' : '#F8D7DA',
          color:      produto.disponivel ? '#155724' : '#721C24',
        }}>
      ...
      {produto.dataValidade && (
        <div style={{
          fontSize: '0.72rem',
          color: alerta.statusValidade === 'vencido'  ? '#B91C1C'
               : alerta.statusValidade === 'vencendo' ? '#C2410C'
               : '#8B6F5E',
      ...
      <div style={styles.acoes}>
        <button
          ...
          onClick={() => onToggleDisponivel(produto)}
          title={produto.disponivel ? 'Desativar' : 'Ativar'}
        >
          {produto.disponivel ? '⏸' : '▶'}
        </button>
        <button style={styles.btnLotes} onClick={() => setLotesAberto(true)}>
          📦 Lotes
        </button>
        <button style={styles.btnEditar} onClick={() => onEditar(produto)}>
          Editar
        </button>
        <button style={styles.btnDeletar} onClick={() => onDeletar(produto.id)}>
          Remover
        </button>
      </div>
      ...
      {/* Modal de lotes */}
      <ModalLotes
        produto={lotesAberto ? produto : null}
        onFechar={() => setLotesAberto(false)}
        onAtualizar={() => {
          onToggleDisponivel(produto);
          setLotesAberto(false);
        }}
      />
    </div>
  );
...
const styles: Record<string, React.CSSProperties> = {
  ...
  btnToggle:    { padding: '0.4rem 0.6rem', background: '#FFF3E0', border: '1px solid #E0C9A8', borderRadius: '0.4rem', cursor: 'pointer', fontSize: '0.85rem', flexShrink: 0 },
  btnLotes:     { flex: 1, padding: '0.45rem', background: '#EFF6FF', border: '1px solid #BAE6FD', borderRadius: '0.4rem', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontSize: '0.82rem', color: '#0369A1', fontWeight: 600 },
