...
export function gerarCupomHTML(venda: Venda): string {
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Courier New', Courier, monospace; font-size: 11px; width: 80mm; padding: 4mm 3mm; background: #fff; color: #000; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        .nome-padaria { font-size: 14px; font-weight: bold; text-align: center; text-transform: uppercase; margin-bottom: 2px; }
        .cnpj { text-align: center; font-size: 10px; margin-bottom: 2px; }
        .nao-fiscal { text-align: center; font-size: 10px; font-weight: bold; border: 1px solid #000; padding: 2px 4px; margin: 4px auto; width: fit-content; letter-spacing: 1px; }
        .linha { border-top: 1px dashed #000; margin: 4px 0; }
        .linha-solida { border-top: 1px solid #000; margin: 4px 0; }
        .info-row { display: flex; justify-content: space-between; font-size: 10px; margin: 1px 0; }
        .secao-titulo { font-weight: bold; font-size: 10px; text-transform: uppercase; margin: 3px 0 2px; }
        .item-nome { font-size: 11px; margin-top: 3px; font-weight: bold; }
        .item-calc { display: flex; justify-content: space-between; font-size: 10px; color: #333; margin-bottom: 2px; padding-left: 4px; }
        .total-row { display: flex; justify-content: space-between; font-size: 13px; font-weight: bold; margin: 3px 0; }
        .pagamento-row { display: flex; justify-content: space-between; font-size: 11px; margin: 2px 0; }
        .troco-row { display: flex; justify-content: space-between; font-size: 12px; font-weight: bold; margin: 2px 0; }
        .mensagem-final { text-align: center; font-size: 11px; margin: 6px 0 4px; font-style: italic; }
        .cupom-id { text-align: center; font-size: 9px; color: #555; margin-top: 3px; }
    ...
    <body>
      ...
      <div class="info-row"><span>Data/Hora:</span><span>${dataHora}</span></div>
      <div class="info-row"><span>Cupom Nº:</span><span>${venda.id.slice(0, 8).toUpperCase()}</span></div>
      ${venda.usuarioNome ? `<div class="info-row"><span>Operador:</span><span>${venda.usuarioNome}</span></div>` : ''}
      ...
      <div class="troco-row"><span>TROCO</span><span>${fmt(venda.troco)}</span></div>` : ''}

      ${venda.observacao ? `
      <div class="linha"></div>
      <div class="info-row"><span>Obs: ${venda.observacao}</span></div>` : ''}
