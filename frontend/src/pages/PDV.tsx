...
export function PDV() {
  ...
  // ── Concluir venda — retorna a venda para o ModalPagamento ─
  const concluirVenda = async (forma: string, valorPago: number, obs: string): Promise<Venda> => {
    if (!venda) throw new Error('Sem venda ativa');
    const resultado = await vendaService.concluir(venda.id, forma, valorPago, obs);
    tocar('venda');
    setModalPagamento(false);
    setCarrinhoAberto(false);
    setSucesso(`Venda concluída!`);
    await iniciarVenda();
    await produtoService.listar({ disponivel: true }).then(r => setProdutos(r.data));
    setTimeout(() => setSucesso(''), 5000);
    return resultado.data;  // ← retorna para o modal exibir tela de sucesso + imprimir
  };

  // ── Leitor de código de barras ────────────────────────────
  useBarcode({
    ativo: barcodeAtivo && !modalPagamento && !cameraAberta,
  ...
  // ── Componente Carrinho reutilizado ───────────────────────
  const carrinhoComponent = venda && (
    <Carrinho
      venda={venda}
      onRemover={removerItem}
      onAlterarQtd={alterarQtd}
      onAlterarPreco={alterarPrecoItem}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
            <button
              style={{ padding: '0.35rem 0.75rem', border: `1.5px solid ${filtroCateg === '' ? '#C8822A' : '#E0C9A8'}`, borderRadius: '999px', cursor: 'pointer', background: filtroCateg === '' ? '#C8822A' : '#FFFBF5', color: filtroCateg === '' ? '#fff' : '#7A5C4E', fontFamily: "'DM Sans', sans-serif", fontSize: '0.82rem', fontWeight: filtroCateg === '' ? 700 : 400 }}
        ...
        {erro && (
          <div
            style={{ background: '#FEF2F2', border: '1px solid #FCA5A5', color: '#B91C1C', borderRadius: '0.5rem', padding: '0.6rem 1rem', fontFamily: "'DM Sans', sans-serif", fontSize: '0.875rem', cursor: 'pointer' }}
            onClick={() => setErro('')}
          >
            {erro} ✕
          </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, minWidth: 0 }}>
            ...
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.78rem', color: barcodeAtivo ? '#065F46' : '#8B6F5E', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {barcodeMsg || (barcodeAtivo ? 'Leitor USB/BT ativo' : 'Leitor USB/BT desativado')}
            </span>
          </div>
        ...
        <div style={{
          ...
          paddingBottom: isMobile ? '8rem' : '1rem',
        }}>
        ...
        >
          ...
          {totalItens > 0 && (
            <span style={{
              ...
              fontSize: '0.72rem', fontWeight: 700, border: '2px solid #FDF6EC',
            }}>
      ...
      {compact && carrinhoAberto && (
        <>
          <div onClick={() => setCarrinhoAberto(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 199 }} />
          <div style={{
            position: 'fixed', bottom: 0, left: 0, right: 0,
            height: isMobile ? '85vh' : '70vh',
            background: '#FFFBF5', borderRadius: '1.25rem 1.25rem 0 0',
            zIndex: 200, boxShadow: '0 -8px 32px rgba(0,0,0,0.2)',
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 1.25rem 0.75rem', borderBottom: '1px solid #F0E4CC' }}>
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.1rem', color: '#2C1A0E', fontWeight: 700 }}>🛒 Carrinho</span>
              <button onClick={() => setCarrinhoAberto(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.1rem', color: '#8B6F5E', padding: '4px 8px' }}>✕</button>
            </div>
